...
- **Ban phases are red everywhere.** The roster draft's ban step was the only one
  ...
  were picking. `CycleMod_DraftCard(idx, isBan, viewerIsP1, spectator)` now
  supplies the button styling for both the picker's view and the spectator view,
  and the race draft's ban step is coloured too. `isBan` is passed in rather than
  read from `g_cycle_draftPhase`, because legacy phase mode leaves that global at
  0 and would otherwise render every button as a ban.
- **Nothing leaves the draft board** (`scripts/DraftStyle.galaxy`, included
  first, before `RaceDraft`). All three drafts used to *hide* an option the
  moment it was banned or taken, so the board shrank and reflowed under the
  players, and a banned option was indistinguishable from one that was never in
  the pool. Every card now stays where it is, restyled and disabled. States:
  `c_draftStateOpen` / `BanTarget` / `Banned` / `Mine` / `Theirs` / `Taken`;
  `DraftStyle_Card(state, tag, name, detail)` renders the two-line card and
  `DraftStyle_IsOpen(state)` gates the button, so no screen can leave a dead
  card clickable. Each state is carried by five redundant channels: the word
  (`BANNED` / `YOURS` / `THEIRS` / `TAKEN`), a glyph (`[X]` / `[+]` / `[O]` /
  `[=]`, plain ASCII so no font is assumed), the button frame (a claimed card is
  disabled, so SC2 drops its lit blue frame to a flat panel — what is still live
  reads from the shape of the grid with no text read at all), brightness, and
  colour last, as both text and a tint on the button itself. Green/orange for
  yours/theirs is exactly the pair a deuteranope cannot separate, which is why
  that pair also differs by word, glyph and brightness.
- **Every card sets `<s val="...">`, and this is the whole ballgame.** A first
  cut relied on inline `<c val>` colour plus a marker, and a playtest screenshot
  showed banned, yours and theirs rendering *identically*. The cause is the
  default label style on a trigger dialog button, `StandardButton`
  (`core.sc2mod`, `UI/FontStyles.SC2Style`):
  `template="HeaderExtendedTemplate" height="18" styleflags="Shadow|Uppercase"
  disabledcolor="#ColorStandardButtonDisabled"`. Three separate problems fall
  out of that one line. (1) `disabledcolor` (= `3d5d7a`) is applied to the whole
  label the instant the button is disabled and **overrides every inline `<c>` in
  the string** — and a claimed card *is* a disabled button, so the colour
  channel was not weak, it was being discarded outright. (2) `Uppercase` forced
  every name to ALL CAPS. (3) `height=18` in the wide `FontHeaderExtended` made
  the text large enough to truncate names (`Veteran Forces` → `Veter...`) and
  pushed line two outside the 56px button, so **the second line never drew at
  all** — every marker that lived there, including the older `BAN ·`, had never
  actually been visible. The fix replaces the style **on the control**, via
  `c_triggerControlPropertyStyle` — what the editor's "Set Dialog Item Style"
  action does (`libNtve_gf_SetDialogItemStyle`). `DraftStyle_InitButton` is
  called on every draft button right after `DialogControlCreate` and sets
  `StandardButtonMini`: `template="StandardTemplate" height="16"
  vjustify="Middle" hjustify="Center"`, with no `disabledcolor` and no
  `Uppercase`. That fixes all three at once — colour survives disabling, names
  stay mixed-case and unabbreviated, both lines draw — and because it keeps the
  same centring `StandardButton` had, swapping the style moves nothing on
  screen. The detail line additionally sets `Standard12` inline just to be
  smaller than the name; the name line is left unwrapped so it inherits the
  control style. Labels (legends, the pool header) take `Standard11` through
  `DraftStyle_Small`, which is what stops the legend truncating. The name
  therefore stays alone on line one and the chip sits on line two.
- **`DraftStyle_ApplyCard(control, viewer, state, card, canClick)`** is the only
  way any screen touches a draft button — text, button tint
  (`c_triggerControlPropertyColor`, a second colour path independent of the text
  style), visibility and enabled all in one place, so no screen can style a card
  one way and gate it another. Per-screen state sources: the race draft reads
  `g_draft_bannedRace`; the modifier draft gained `g_cycle_modState[15]`
  (`c_cycleSlot*`), since `g_cycle_taken` says only *that* a modifier is claimed,
  not by whom; the roster draft already had `g_roster_p{1,2}{Banned,Picked}` and
  reads them through `RosterDraft_SlotState`. "Yours" vs "theirs" is per viewer,
  so the waiting player sees the same board with the two swapped, and spectators
  (on neither side) get `P1` / `P2` instead. Click handlers reject a dead card
  as well — `RaceDraft_OnClicked` gained an explicit banned-race guard, the
  other two already had one.
