...
- `const fixed c_cycleActivationDelay = 180.0` — modifiers stay **dormant** until
  ...
  **Opening vision:** until that same moment, `CycleMod_UpdateOpeningVision`
  ...
  mod (cooldowns, Entrenchment/Overwatch waits, Salvage) is likewise game seconds.
...
| Item | Notes |
|---|---|
| Modifier balance | `MoveSpeedMultiplier=1.7`, Veteran `1.03 all speeds/rates/regen ×15`, ability cooldowns (12s Blink / 15s Medivac Boost, both uncosted), etc. are first-pass. Same for the new four: Shared Damage radius 3 with no cap on Y, Auto Refineries' 2.1s trip interval, Salvage's 5s timer. |
